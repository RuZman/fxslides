...
public class MotionRegistry implements LeapListener {
		...
		if (frame.hands().count() >= LeapApp.getMinimumHandNumber()) {
			...
			for (Hand hand : frame.hands()) {
				...
				if (extendedHand == null && hands.size() < LeapApp.getMaximumHandNumber()) {
					extendedHand = new ExtendedHand(hand.id(), LeapApp.getTrackingBox());
				...
				if (extendedHand != null) {
					if (handCount++ < LeapApp.getMaximumHandNumber()) {
						extendedHand.update(frame, frame.hand(extendedHand.id()).palmPosition(),
								frame.hand(extendedHand.id()).stabilizedPalmPosition(), frame.hand(extendedHand.id())
										.palmVelocity());
					}
				}
			}
	...
	public void addListener(IListener listener) {
		if (listener instanceof PointZoneListener || listener instanceof PointMotionListener
