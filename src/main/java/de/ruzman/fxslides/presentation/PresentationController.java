...
public class PresentationController {
	...
	private SimplePalmDragAndDropGestureProvider dragAndDropProvider = new SimplePalmDragAndDropGestureProvider();
	...
	public void onUpdate(SkeletonEvent event) {
		if(event.getGestures().contains("dragStart")) {
			onDragStart((int) event.getSkeleton().getHands().get(0).getPalmPosition().get().getScreenPosition().getX());
		}
		
		if(event.getGestures().contains("drag")) {
			onDragUpdate((int) event.getSkeleton().getHands().get(0).getPalmPosition().get().getScreenPosition().getX());
		}
	}
